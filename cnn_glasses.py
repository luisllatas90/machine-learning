...
class Model:
  ...
  def print_inside_cnn(self,img,label):
    ...
    for index in range(32):
      d = c[0,index,:,:]
      #d = scipy.misc.imresize(d, (56, 56)) #luis llatas
      d = np.array(Image.fromarray(d).resize((56, 56))).astype(np.uint8) #luis llatas
      path = base_path+"layer_1_"+str(index)+".png"
      save_img(d, path_to=path) #luis llatas
      #Image.SAVE(d,path_to=path) #luis llatas
